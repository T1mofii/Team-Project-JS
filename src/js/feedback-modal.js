...
async function handleFormSubmit(event) {
  ...
  if (!data.name || !data.descr || !data.rating) {
    warningFeedback('Будь ласка, заповніть всі поля!');
    return;
  }
  if (data.name.length < 2 || data.name.length > 16) {
    warningFeedback('Ім\'я повинно містити від 2 до 16 символів.');
    return;
  }
  if (data.descr.length < 10 || data.descr.length > 512) {
    warningFeedback('Повідомлення повинно містити від 10 до 512 символів.');
    return;
  }
  ...
  try {
    ...
    errorFeedback('Something went wrong. Please try again later.');
